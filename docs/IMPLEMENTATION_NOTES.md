...
| 19 | The health document is served at `/api/health`, with `/healthz` kept as a local alias. | Cloud Run's front end answers `/healthz` itself with a 404, so the self-check result was invisible on the hosted URL. |
| 20 | The page has a product title, "Agentic Database Security Remediation", with the tagline "From finding to verified recovery". The honesty banner reads "Demo mode: Recorded agents • Simulated PostgreSQL target • Demo personas • Live tools, policy, guards, workflow, and fault handling". | Review feedback. The wording keeps the four facts invariant I13 requires. |
